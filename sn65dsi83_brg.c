...
static int sn65dsi83_brg_read_all(struct sn65dsi83_brg *brg)
{
    u8 reg;
    u8 regval = 0;
    struct i2c_client *client = I2C_CLIENT(brg);

    for(reg=0x00;reg<0x3e;reg++){
        regval = SN65DSI83_READ(reg);
        dev_info(&client->dev, "addr: 0x%02x, 0x%02x\n",reg,regval);
    }
...
static int sn65dsi83_brg_configure(struct sn65dsi83_brg *brg)
{
    ...
    if(brg->dsi_packet_type == 100){
        // TEST MODE
        dsi_clk = (((PIXCLK * BPP(brg)) / DSI_LANES(brg)) >> 1);
    }
    else if(brg->dsi_bpp == 18 && brg->dsi_packet_type == 0){ 
         // DSI 18bpp and loosely packed       
        dsi_clk = (((PIXCLK * 24) / DSI_LANES(brg))>>1);
    }else{                              
         // DSI 18bpp and tightly packed, or DSI 24bpp                          
        dsi_clk = (((PIXCLK * brg->dsi_bpp) / DSI_LANES(brg))>>1);
    }
    ...
    SN65DSI83_WRITE(SN65DSI83_SOFT_RESET,0x00);         // No Action
    SN65DSI83_WRITE(SN65DSI83_PLL_EN,0x00);             // Disable PLL
    ...
    regval |= (1 << LVDS_LINK_CFG_SHIFT);           // single
...
#ifdef TOY_MODIFICATION   
    regval = TEST_MODE(brg)<<4;
    SN65DSI83_WRITE(SN65DSI83_TEST_PATTERN,regval);           // enable test mode for channel A
#endif
